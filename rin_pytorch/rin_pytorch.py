...
class RIN(nn.Module):
    def __init__(
          ...
          # how many self attentions for the latent per each round of cross attending from pixel space to latents and back
          dim_latent=None,  # will default to image dim (dim)
          num_latents=256,
          # they still had to use a fair amount of latents for good results (256), in line with the Perceiver line of papers from Deepmind
        ...
        self.blocks = nn.ModuleList(
            [RINBlock(dim, dim_latent=dim_latent, latent_self_attn_depth=latent_self_attn_depth, **attn_kwargs) for _ in
             range(depth)])
...
class GaussianDiffusion(nn.Module):
    def __init__(
          self,
          model: Union[OptimizedModule, RIN],
          *,
          timesteps=1000,
          use_ddim=True,
    ...
    def ddpm_sample(self, shape, time_difference=None):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step', total=self.timesteps):
            ...
            model_output, last_latents = self.model(maybe_normalized_img, noise_cond, x_start, last_latents,
                                                    return_latents=True)
    ...
    def ddim_sample(self, shape, time_difference=None):
        ...
        for times, times_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            maybe_normalized_img = self.maybe_normalize_img_variance(img)
            model_output, last_latents = self.model(maybe_normalized_img, times, x_start, last_latents,
                                                    return_latents=True)
...
class Trainer(object):
    def __init__(
          ...
          *,
          ...
          num_samples=25,  # for sampling/logging
